...
import { ApplicationSchema, MenusSchema } from '../config/schema/schema';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  return (
    <Box
      ...
      {MenusSchema.map((link) => (
        <NavItem key={link.label} icon={link.icon} href={link.path}>
          {link.label}
        </NavItem>
      ))}
    </Box>
...
const NavItem = ({ icon, children, href, ...rest }: NavItemProps) => {
  return (
    <Link as={LinkRouter} to={href || ''} style={{ textDecoration: 'none' }} _focus={{ boxShadow: 'none' }}>
      <Flex
        align='center'
        p='4'
        mx='4'
