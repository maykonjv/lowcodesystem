...
<form   action="builder" method="post" id="modalDeleteFormForm">
    <input type="hidden" name="session" value="form-sql-delete"/>
    <input type="hidden" name="page" value="<%=((Page) request.getAttribute("p")).getId()%>"/>
    <input type="hidden" name="action" value="<%=request.getAttribute("action")%>"/>
    <div class="modal fade" id="modalSQLDelete" data-coreui-backdrop="static" data-coreui-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="mb-3 col-md-3">
                            ...
                            <select multiple class="form-control" style="height: 375px;" ondblclick="$('#sqlDelete').val($('#sqlDelete').val() + $(this).val())">
                        ...
                        <div class="mb-3 col-md-9">
                            <label for="sqlDelete" class="form-label">SQL de exclusão do registro</label>
                            <span title="Informe um sql válido para exclur o registro. Ex. DELETE FROM tabela WHERE id=\#{id}">
                            ...
                            <textarea id="sqlDelete" name="sqlDelete" class="form-control"  placeholder="Informe o sql para atualizar o registro" rows="15"><%= ((Page) request.getAttribute("p")).getSqlDelete() %></textarea>
                        </div>

                        <div class="mb-3 col-md-4">
                            <label for="sqlDeleteDS" class="form-label">Datasource</label>
                            <select id="sqlDeleteDS" name="sqlDeleteDS" class="form-select" size="1">	
                                <% for (String key : DataSource.database.keySet()) {
                                        out.println("<option value=\"" + key + "\" " + (((Page) request.getAttribute("p")).getSqlDeleteDS().equals(key) ? "selected" : "") + ">" + key + "</option>");
                                    }%>
                            </select>
                        </div>
                    </div>
